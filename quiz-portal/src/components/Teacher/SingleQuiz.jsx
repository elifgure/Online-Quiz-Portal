          ...
          <ul className="space-y-8">
            {quiz.elements.map((el, idx) => {
              ...
              return (
                <li
                  key={el.id || idx}
                  className="bg-purple-50/60 rounded-lg p-4 border border-purple-100"
                >
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-medium text-[#044c5c]">
                      {idx + 1}.
                    </span>
                    <span className="font-semibold text-[#044c5c]">
                      {el.label || "(Başlıksız Soru)"}
                    </span>
                  </div>
                  <div className="mt-2">
                    {ElementComponent ? (
                      <ElementComponent {...el} isPreview={true} />
                    ) : (
                      <input
