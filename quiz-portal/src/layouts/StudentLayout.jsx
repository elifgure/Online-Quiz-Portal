    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-white to-orange-100">
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2 bg-purple-50 px-3 py-2 rounded-full border border-purple-200">
                ...
                <span className="text-sm font-medium text-[#044c5c]">Öğrenci Adı</span>
              </div>
              <button
                onClick={()=>{handleLogout()}}
                className="flex items-center space-x-2 px-4 py-2 text-sm font-medium text-orange-600 hover:text-orange-700 hover:bg-orange-50 rounded-lg transition-colors duration-200 border border-orange-200"
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            >
              <div className="p-8">
                <div className="flex items-center justify-between mb-4">
                  <div className={`${index === 0 ? 'bg-gradient-to-r from-purple-500 to-pink-500' : 'bg-gradient-to-r from-orange-500 to-red-500'} p-3 rounded-xl group-hover:scale-110 transition-transform duration-300`}>
                    <item.icon className="h-8 w-8 text-white" />
                  </div>
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <div className={`w-2 h-2 ${index === 0 ? 'bg-purple-500' : 'bg-orange-500'} rounded-full`}></div>
                  </div>
                </div>
                ...
                <p className="text-[#37747c] text-sm">
                  {item.title === "Sınavlarım" ? "Sınavlarınızı görüntüleyin ve yönetin" : "Performans raporlarınızı inceleyin"}
                </p>
              </div>
              <div className={`h-1 ${index === 0 ? 'bg-gradient-to-r from-purple-500 to-pink-500' : 'bg-gradient-to-r from-orange-500 to-red-500'} rounded-b-2xl transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left`}></div>
            </div>
          ))}
        ...
        <div className="mt-12 relative z-10">
          <h3 className="text-2xl font-bold text-[#044c5c] mb-6">Son Aktiviteler</h3>
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-purple-200">
            <div className="p-6">
              <div className="space-y-4">
                {[
                  { activity: "Matematik Sınavı tamamlandı", time: "2 saat önce", score: "92%", color: "purple" },
                  { activity: "Fizik Sınavı başlatıldı", time: "1 gün önce", score: "88%", color: "orange" },
                  { activity: "Kimya Raporu görüntülendi", time: "3 gün önce", score: "76%", color: "pink" }
                ].map((item, index) => (
                  <div key={index} className="flex items-center justify-between p-4 bg-white/60 backdrop-blur-sm rounded-xl hover:bg-purple-50 transition-colors duration-200 border border-purple-100">
                    <div className="flex items-center space-x-4">
                      <div className={`w-3 h-3 ${item.color === 'purple' ? 'bg-purple-500' : item.color === 'orange' ? 'bg-orange-500' : 'bg-pink-500'} rounded-full`}></div>
                      <div>
                        <p className="font-medium text-[#044c5c]">{item.activity}</p>
